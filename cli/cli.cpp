...
namespace cppsdkcli {
	ResponseCode CLI::InitializeTLS() {
		...
		p_network_connection_ = std::shared_ptr<NetworkConnection>(
			...
			if(nullptr == p_network_connection_) {
				AWS_LOG_ERROR(CLI_LOG_TAG, "Failed to initialize Network Connection with rc : %d", static_cast<int>(rc));
	...
	ResponseCode CLI::InitializeCliConfig() {
		...
		if(ResponseCode::SUCCESS != rc) {
			AWS_LOG_ERROR(CLI_LOG_TAG, "Initialize Test Config Failed with rc : %d", static_cast<int>(rc));
		}
		return rc;
	}
	...
	ResponseCode CLI::RunPublish(int msg_count) {
		...
		do {
			...
			std::cout << "Publish Response : " << static_cast<int>(rc) << std::endl;
	...
	ResponseCode CLI::SubscribeCallback(util::String topic_name, util::String payload, std::shared_ptr<mqtt::SubscriptionHandlerContextData> p_app_handler_data) {
	...
	ResponseCode CLI::Subscribe(std::unique_ptr<Utf8String> p_topic_name, mqtt::QoS qos) {
		...
		std::shared_ptr<mqtt::Subscription> p_subscription = mqtt::Subscription::Create(std::move(p_topic_name), qos, &SubscribeCallback, nullptr);
	...
	ResponseCode CLI::RunCLI() {
		...
		if(awsiotsdk::ResponseCode::MQTT_CONNACK_CONNECTION_ACCEPTED != rc) {
			AWS_LOG_ERROR(CLI_LOG_TAG, "Connect failed with RC : ", static_cast<int>(rc));
			return rc;
		}
