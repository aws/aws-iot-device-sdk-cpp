...
namespace awsiotsdk {
	namespace util {
		ResponseCode JsonParser::InitializeFromJsonFile(JsonDocument &json_document, const util::String &config_file_path) {
			if(0 == config_file_path.length()) {
				return ResponseCode::NULL_VALUE_ERROR;
			}

			std::unique_ptr<FILE, int(*)(FILE*)> config_file = std::unique_ptr<FILE, int(*)(FILE*)>(fopen(config_file_path.c_str(), "rb"), fclose);
			if(nullptr == config_file) {
				return ResponseCode::FILE_OPEN_ERROR;
			}

			std::streampos fsize = 0;
			std::ifstream config_file_stream(config_file_path, std::ios::binary);
			...
			rapidjson::FileReadStream fileReadStream(config_file.get(), read_buffer.get(), static_cast<size_t>(fsize));
		...
		ResponseCode JsonParser::InitializeFromJsonString(JsonDocument &json_document, const util::String &config_json_string) {
		...
		ResponseCode JsonParser::GetCStringValue(const JsonDocument &json_document, const char *key, char *value, uint16_t max_string_len) {
			if(nullptr == value) {
				return ResponseCode::NULL_VALUE_ERROR;
			}
		...
		ResponseCode JsonParser::GetStringValue(const JsonDocument &json_document, const char *key, util::String &value) {
			if(!json_document.HasMember(key)) {
				return ResponseCode::JSON_PARSE_KEY_NOT_FOUND_ERROR;
			}
		...
		ResponseCode JsonParser::MergeValues(JsonValue &target, JsonValue &source, JsonValue::AllocatorType& allocator) {
		...
		// be to sort the json and bring it down to O(nlog(n)). But changing the json structure
		// might not be desired. Provide a second implementation that does this with an option
		ResponseCode JsonParser::DiffValues(JsonValue &target_doc, JsonValue &old_doc, JsonValue &new_doc, JsonValue::AllocatorType& allocator) {
