...
namespace awsiotsdk {
	namespace mqtt {
		...
		std::shared_ptr<Subscription> Subscription::Create(std::unique_ptr<Utf8String> p_topic_name, QoS max_qos, ApplicationCallbackHandlerPtr p_app_handler, std::shared_ptr<SubscriptionHandlerContextData> p_app_handler_data) {
			if(nullptr == p_topic_name || nullptr == p_app_handler) {
				return nullptr;
			}

			return std::shared_ptr<Subscription>(new Subscription(std::move(p_topic_name), max_qos, p_app_handler, p_app_handler_data));
		}

		Subscription::Subscription(std::unique_ptr<Utf8String> p_topic_name, QoS max_qos, ApplicationCallbackHandlerPtr p_app_handler, std::shared_ptr<SubscriptionHandlerContextData> p_app_handler_data) {
			is_active_ = false;
			index_in_packet_ = 0;
			packet_id_ = 0;
