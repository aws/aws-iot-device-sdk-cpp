	...
	namespace mqtt {
		...
		SubscribePacket::SubscribePacket(util::Vector<std::shared_ptr<Subscription>> subscription_list) {
			...
			for(itr = subscription_list.begin(); itr < subscription_list.end(); ++itr) {
				packet_size_ = packet_size_ + (*itr)->GetTopicNameLength() + 2 + 1; // 2 bytes for topic length, 1 for QoS
			}
		...
		ResponseCode SubscribeActionAsync::PerformAction(std::shared_ptr<NetworkConnection> p_network_connection, std::shared_ptr<ActionData> p_action_data) {
			...
			std::shared_ptr<SubscribePacket> p_subscribe_packet = std::dynamic_pointer_cast<SubscribePacket>(p_action_data);
			...
			if(ResponseCode::SUCCESS != rc) {
				AWS_LOG_ERROR(SUBSCRIBE_ACTION_LOG_TAG, "Subscribe Write to Network Failed with return code : %d",
							  static_cast<int>(rc));
				// Remove acks
				for(itr = p_subscribe_packet->subscription_list_.begin(); itr < p_subscribe_packet->subscription_list_.end(); ++itr) {
					util::String topic_name = (*itr)->GetTopicName()->ToStdString();
		...
		ResponseCode UnsubscribeActionAsync::PerformAction(std::shared_ptr<NetworkConnection> p_network_connection, std::shared_ptr<ActionData> p_action_data) {
			std::shared_ptr<UnsubscribePacket> p_unsubscribe_packet = std::dynamic_pointer_cast<UnsubscribePacket>(p_action_data);
			...
			if(nullptr != p_unsubscribe_packet->p_async_ack_handler_) {
				rc = p_client_state_->RegisterPendingAck(p_unsubscribe_packet->GetPacketId(), p_unsubscribe_packet->p_async_ack_handler_);
				if(ResponseCode::SUCCESS != rc) {
			...
			rc = WriteToNetworkBuffer(p_network_connection, packet_data);
			if(ResponseCode::SUCCESS != rc) {
				AWS_LOG_ERROR(UNSUBSCRIBE_ACTION_LOG_TAG, "Publish Write to Network Failed with return code : %d",
							  static_cast<int>(rc));
				if(is_ack_registered) {
					p_client_state_->DeletePendingAck(packet_id);
				}
