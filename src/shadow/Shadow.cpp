...
namespace awsiotsdk {
	Shadow::Shadow(std::shared_ptr<MqttClient> p_mqtt_client, std::chrono::milliseconds mqtt_command_timeout,
		...
		client_token_ = client_token_prefix_ + "_" + std::to_string(std::chrono::high_resolution_clock::now().time_since_epoch().count());
		cur_device_state_document_[SHADOW_DOCUMENT_CLIENT_TOKEN_KEY].SetString(client_token_.c_str(), cur_server_state_document_.GetAllocator());
	...
	std::unique_ptr<Shadow> Shadow::Create(std::shared_ptr<MqttClient> p_mqtt_client,
		...
		return std::unique_ptr<Shadow>(new Shadow(p_mqtt_client, mqtt_command_timeout, thing_name, client_token_prefix));
	}
	...
	ResponseCode Shadow::SubscriptionHandler(util::String topic_name, util::String payload,
		...
		if(ResponseCode::SUCCESS == rc) {
			...
			AWS_LOG_ERROR(SHADOW_LOG_TAG, "\"Error in Parsing, rc : %d parse error code : %d, offset : %u",
						  static_cast<int>(rc),
	...
	ResponseCode Shadow::UpdateDeviceShadow(util::JsonDocument &document) {
		...
		return util::JsonParser::MergeValues(cur_device_state_document_, document, cur_device_state_document_.GetAllocator());
	}

	util::JsonDocument Shadow::GetDeviceReported() {
		util::JsonDocument reported;
		reported.CopyFrom(cur_device_state_document_[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_REPORTED_KEY], reported.GetAllocator());
		return std::move(reported);
	}

	util::JsonDocument Shadow::GetDeviceDesired() {
		util::JsonDocument desired;
		desired.CopyFrom(cur_device_state_document_[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_DESIRED_KEY], desired.GetAllocator());
	...
	util::JsonDocument Shadow::GetServerReported() {
		util::JsonDocument reported;
		reported.CopyFrom(cur_server_state_document_[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_REPORTED_KEY], reported.GetAllocator());
		return std::move(reported);
	}

	util::JsonDocument Shadow::GetServerDesired() {
		util::JsonDocument desired;
		desired.CopyFrom(cur_server_state_document_[SHADOW_DOCUMENT_STATE_KEY][SHADOW_DOCUMENT_DESIRED_KEY], desired.GetAllocator());
		return std::move(desired);
	}
	...
	ResponseCode Shadow::PerformGetAsync() {
		...
		util::String payload = "";
		rc = p_mqtt_client_->Publish(Utf8String::Create(shadow_topic_get_), false, false, mqtt::QoS::QOS1, payload,
	...
	ResponseCode Shadow::PerformUpdateAsync() {
		...
		rc = p_mqtt_client_->Publish(Utf8String::Create(shadow_topic_update_), false, false, mqtt::QoS::QOS1,
	...
	ResponseCode Shadow::PerformDeleteAsync() {
		...
		util::String payload = "";
		rc = p_mqtt_client_->Publish(Utf8String::Create(shadow_topic_delete_), false, false, mqtt::QoS::QOS1, payload,
										 mqtt_command_timeout_);
		return rc;
	}
