	...
	namespace tests {
		namespace integration {
			ResponseCode AutoReconnect::RunPublish(int msg_count) {
				std::cout<<std::endl<<"******************************Entering Publish!!**************************"<<std::endl;
				ResponseCode rc;
			...
			ResponseCode AutoReconnect::SubscribeCallback(util::String topic_name, util::String payload, std::shared_ptr<mqtt::SubscriptionHandlerContextData> p_app_handler_data) {
			...
			ResponseCode AutoReconnect::LifecycleSubscribeCallback(util::String topic_name, util::String payload, std::shared_ptr<mqtt::SubscriptionHandlerContextData> p_app_handler_data) {
			...
			ResponseCode AutoReconnect::Subscribe() {
				...
				mqtt::Subscription::ApplicationCallbackHandlerPtr p_sub_handler = std::bind(&AutoReconnect::SubscribeCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
				mqtt::Subscription::ApplicationCallbackHandlerPtr p_lifecycle_sub_handler = std::bind(&AutoReconnect::LifecycleSubscribeCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
				...
				std::shared_ptr<mqtt::Subscription> p_lifecycle_subscription = mqtt::Subscription::Create(std::move(p_lifecycle_topic_name), mqtt::QoS::QOS0, p_lifecycle_sub_handler, nullptr);
			...
			if(nullptr == p_network_connection_) {
				AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Failed to initialize Network Connection with rc : %d", static_cast<int>(rc));
			...
			ResponseCode AutoReconnect::RunTest() {
				...
				do {
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Failed to initialize TLS layer with return code : %d",
									  static_cast<int>(rc));
						break;
					}
					...
					if(ResponseCode::MQTT_CONNACK_CONNECTION_ACCEPTED != rc) {
						AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "MQTT Connect failed with return code : %d!!",
									  static_cast<int>(rc));
						return rc;
					}
					...
					rc = Subscribe();
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Subscribe failed with return code : %d",
									  static_cast<int>(rc));
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Publish runner failed with return code : %d",
									  static_cast<int>(rc));
						p_iot_client_->Disconnect(ConfigCommon::mqtt_command_timeout_);
						break;
					}
...

					{
						std::unique_lock<std::mutex> block_handler_lock(waiting_for_sub_lock_);
						{
							p_iot_client_->SetMinReconnectBackoffTimeout(std::chrono::seconds(NETWORK_RECONNECT_BACKOFF_TIMER_MIN));
							p_iot_client_->SetMaxReconnectBackoffTimeout(std::chrono::seconds(NETWORK_RECONNECT_BACKOFF_TIMER_MAX));
						...
						std::this_thread::sleep_for(std::chrono::milliseconds(100));
						sub_lifecycle_wait_.wait_for(block_handler_lock, std::chrono::seconds(NETWORK_RECONNECT_BACKOFF_TIMER_MAX * 2));
					}
					...
					if(p_iot_client_->IsConnected()) {
						...
						rc = RunPublish(SDK_ACR_TEST_MSG_COUNT);
						if(ResponseCode::SUCCESS != rc) {
							AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG,
										  "Publish runner failed with return code : %d",
										  static_cast<int>(rc));
							p_iot_client_->Disconnect(ConfigCommon::mqtt_command_timeout_);
							break;
						}
						...
						if(ResponseCode::SUCCESS != rc) {
							AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Unsubscribe failed with return code : %d",
										  static_cast<int>(rc));
							p_iot_client_->Disconnect(ConfigCommon::mqtt_command_timeout_);
							break;
						}
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(ARC_INTEGRATION_TEST_TAG, "Disconnect failed with return code : %d",
									  static_cast<int>(rc));
						break;
					}
					ran_all_tests = true;
				...
				if((ResponseCode::FAILURE == rc) || !ran_all_tests || (cur_pending_messages_ > 0 || total_published_messages_ == 0)) {
