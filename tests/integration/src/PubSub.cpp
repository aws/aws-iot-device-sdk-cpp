	...
	namespace tests {
		namespace integration {
			ResponseCode PubSub::RunPublish(int msg_count) {
				std::cout<<std::endl<<"******************************Entering Publish with large payload!!**************************"<<std::endl;
				ResponseCode rc;
			...
			ResponseCode PubSub::InitializeTLS() {
				...
				std::shared_ptr<network::MbedTLSConnection> p_network_connection =
					std::make_shared<network::MbedTLSConnection>(ConfigCommon::endpoint_, ConfigCommon::endpoint_port_,
			...
			ResponseCode PubSub::RunTest() {
				...
				do {
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "Failed to initialize TLS layer with return code : %d",
									  static_cast<int>(rc));
						break;
					}
					...
					if(ResponseCode::MQTT_CONNACK_CONNECTION_ACCEPTED != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "MQTT Connect failed with return code : %d!!",
									  static_cast<int>(rc));
						return rc;
					}
					...
					rc = Subscribe();
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "Subscribe failed with return code : %d",
									  static_cast<int>(rc));
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "Publish runner failed with return code : %d",
									  static_cast<int>(rc));
						p_iot_client_->Disconnect(ConfigCommon::mqtt_command_timeout_);
						break;
					}
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "Unsubscribe failed with return code : %d",
									  static_cast<int>(rc));
						p_iot_client_->Disconnect(ConfigCommon::mqtt_command_timeout_);
						break;
					}
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(PUBSUB_INTEGRATION_TEST_TAG, "Disconnect failed with return code : %d", static_cast<int>(rc));
						break;
					}
					ran_all_tests = true;
				} while(false);
				...
				if((ResponseCode::FAILURE == rc) || !ran_all_tests || (cur_pending_messages_ != 0 || total_published_messages_ == 0)) {
					std::cout<<"Test Failed!!!! See above output for details!! Please be aware that this test may fail if an Autoreconnect event occurred in between this run!!"<<std::endl;
					std::cout<<"**********************************************************"<<std::endl;
