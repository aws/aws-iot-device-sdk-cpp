	...
	namespace tests {
		namespace integration {
			ResponseCode MultipleClients::RunPublish(int msg_count) {
				...
				} while(itr <= 2 * msg_count && (ResponseCode::SUCCESS == rc || ResponseCode::ACTION_QUEUE_FULL == rc));

				return rc;
			}

			ResponseCode MultipleClients::SubscribeCallback(util::String topic_name, util::String payload, std::shared_ptr<mqtt::SubscriptionHandlerContextData> p_app_handler_data) {
			...
			ResponseCode MultipleClients::Subscribe() {
				...
				mqtt::Subscription::ApplicationCallbackHandlerPtr p_sub_handler = std::bind(&MultipleClients::SubscribeCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
			...
			ResponseCode MultipleClients::Unsubscribe() {
				...
				ResponseCode rc = p_iot_client_1_->Unsubscribe(std::move(topic_vector), ConfigCommon::mqtt_command_timeout_);
			...
			if(nullptr == p_network_connection) {
				AWS_LOG_ERROR(MULTIPLE_CLIENTS_INTEGRATION_TEST_TAG, "Failed to initialize Network Connection with rc : %d", static_cast<int>(rc));
			...
			ResponseCode MultipleClients::RunTest() {
				...
				rc = Subscribe();
				if(ResponseCode::SUCCESS != rc) {
					AWS_LOG_ERROR(MULTIPLE_CLIENTS_INTEGRATION_TEST_TAG, "Subscribe failed with return code : %d", static_cast<int>(rc));
				} else {
					...
					if(ResponseCode::SUCCESS != rc) {
						...
						AWS_LOG_ERROR(MULTIPLE_CLIENTS_INTEGRATION_TEST_TAG, "Publish runner failed with return code : %d",
									  static_cast<int>(rc));
					...
					if(ResponseCode::SUCCESS != rc) {
						AWS_LOG_ERROR(MULTIPLE_CLIENTS_INTEGRATION_TEST_TAG, "Unsubscribe failed with return code : %d",
									  static_cast<int>(rc));
					}
				}
				...
				if(ResponseCode::SUCCESS != rc) {
					AWS_LOG_ERROR(MULTIPLE_CLIENTS_INTEGRATION_TEST_TAG, "Disconnect failed with return code : %d", static_cast<int>(rc));
				}
				...
				if(cur_pending_messages_ > 0) {
					std::cout<<"Test Failed!!!! Please be aware that this test may fail if an Autoreconnect event occurred in between this run"<<std::endl;
					std::cout<<"**********************************************************"<<std::endl;
