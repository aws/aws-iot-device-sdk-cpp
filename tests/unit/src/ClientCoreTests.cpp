		...
		namespace unit {
			...
			protected:
				...
				public:
					...
					TestAction(std::shared_ptr<ClientCoreState> p_client_state) : Action(ActionType::RESERVED_ACTION, "Test Action") {
				...
				ClientCoreTester() {
					...
					std::shared_ptr<NetworkConnection> p_network_connection = std::make_shared<tests::mocks::MockNetworkConnection>();
			...
			std::unique_ptr<Action> ClientCoreTester::TestAction::Create(std::shared_ptr<ActionState> p_action_state) {
				std::shared_ptr<ClientCoreState> p_client_state = std::dynamic_pointer_cast<ClientCoreState>(p_action_state);
			...
			ResponseCode ClientCoreTester::TestAction::PerformAction(std::shared_ptr<NetworkConnection> p_network_connection, std::shared_ptr<ActionData> p_action_data) {
				std::shared_ptr<TestActionData> p_test_action_data = std::dynamic_pointer_cast<TestActionData>(p_action_data);
			...
			TEST_F(ClientCoreTester, RegisterOnceSuccess) {
				...
				rc = p_client_core_->PerformAction(ActionType::RESERVED_ACTION, p_test_action_data, std::chrono::milliseconds(200));
				EXPECT_EQ(ResponseCode::SUCCESS, rc);
				...
				EXPECT_EQ(ResponseCode::SUCCESS, rc);
				rc = p_client_core_->PerformAction(ActionType::PUBACK, p_test_action_data_puback, std::chrono::milliseconds(200));
				EXPECT_EQ(ResponseCode::SUCCESS, rc);
			...
			TEST_F(ClientCoreTester, TestAsyncFailOnUnregistered) {
				...
				ResponseCode rc = p_client_core_->PerformAction(ActionType::RESERVED_ACTION, p_test_action_data, std::chrono::milliseconds(200));
