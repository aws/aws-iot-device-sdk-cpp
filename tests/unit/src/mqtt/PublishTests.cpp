	...
	namespace tests {
		namespace unit {
			...
			TEST_F(PublishActionTester, PubackActiontest) {
				...
				EXPECT_CALL(*p_network_mock_, WriteInternalProxy(::testing::_, ::testing::_)).WillOnce(::testing::DoAll(::testing::SetArgReferee<1>(p_puback_packet->Size()), ::testing::Return(ResponseCode::SUCCESS)));
			...
			TEST_F(PublishActionTester, PublishQoS0ActionTest) {
				...
				util::String written_payload((char *)p_last_msg, calculated_rem_len - written_topic_name->Length() - 2);
				...
				EXPECT_EQ(test_topic_, written_topic_name->ToStdString());
				written_payload = util::String((char *)p_last_msg, calculated_rem_len - written_topic_name->Length() - 2);
				EXPECT_EQ(test_payload_, written_payload);
			}
			...
			TEST_F(PublishActionTester, PublishQoS1ActionDupFalseTest) {
				...
				ActionData::AsyncAckNotificationHandlerPtr p_async_ack_handler = std::bind(&PublishActionTester::AsyncAckHandler, this, std::placeholders::_1, std::placeholders::_2);
				...
				util::String written_payload((char *)p_last_msg, calculated_rem_len - written_topic_name->Length() - 4);
				EXPECT_EQ(test_payload_, written_payload);
				...
				written_payload = util::String((char *)p_last_msg, calculated_rem_len - written_topic_name->Length() - 4);
				EXPECT_EQ(test_payload_, written_payload);
