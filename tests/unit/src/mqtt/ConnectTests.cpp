...
namespace awsiotsdk {
	namespace tests {
		namespace unit {
			...
			const std::chrono::seconds ConnectDisconnectActionTester::keep_alive_timeout_ = std::chrono::seconds(KEEP_ALIVE_TIMEOUT_SECS);
			...
			TEST_F(ConnectDisconnectActionTester, ConnectActionTestNoWillMessage) {
				...
				std::shared_ptr<mqtt::ConnectPacket> p_connect_packet = mqtt::ConnectPacket::Create(true, mqtt::Version::MQTT_3_1_1, keep_alive_timeout_, Utf8String::Create(test_client_id_), nullptr, nullptr, nullptr);
				...
				EXPECT_CALL(*p_network_mock_, WriteInternalProxy(::testing::_, ::testing::_)).WillOnce(::testing::DoAll(::testing::SetArgReferee<1>(p_connect_packet->Size()), ::testing::Return(ResponseCode::SUCCESS)));
			...
			TEST_F(ConnectDisconnectActionTester, ConnectActionTestWithWillMessage) {
				...
				std::unique_ptr<Action> p_connect_action = mqtt::ConnectActionAsync::Create(p_core_state_);
				std::unique_ptr<mqtt::WillOptions> p_will_options = mqtt::WillOptions::Create(false, mqtt::QoS::QOS0, Utf8String::Create(test_topic_name_), payload);
				std::shared_ptr<mqtt::ConnectPacket> p_connect_packet = mqtt::ConnectPacket::Create(true, mqtt::Version::MQTT_3_1_1, keep_alive_timeout_, Utf8String::Create(test_client_id_), nullptr, nullptr, std::move(p_will_options));
				...
				//Rem len = Variable header (10 bytes) + 2 + test_client_id_ length
				size_t expected_rem_len = 10 + 2 + test_client_id_.length() + 2 + test_topic_name_.length() + 2 + test_payload_.length();
				size_t written_rem_len = TestHelper::ParseRemLenFromBuffer(&p_last_msg);
				EXPECT_EQ(expected_rem_len, written_rem_len);
				...
				//Rem len = Variable header (10 bytes) + 2 + test_client_id_ length
				expected_rem_len = 10 + 2 + test_client_id_.length() + 2 + test_topic_name_.length() + 2 + test_payload_.length();
				written_rem_len = TestHelper::ParseRemLenFromBuffer(&p_last_msg);
				EXPECT_EQ(expected_rem_len, written_rem_len);
			...
			TEST_F(ConnectDisconnectActionTester, DisconnectActionTest) {
				...
				EXPECT_CALL(*p_network_mock_, WriteInternalProxy(::testing::_, ::testing::_)).WillOnce(::testing::DoAll(::testing::SetArgReferee<1>(p_disconnect_packet->Size()), ::testing::Return(ResponseCode::SUCCESS)));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackAcceptedTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::CONNECTION_ACCEPTED));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackRejectedUnacceptableProtocolTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::UNACCEPTABLE_PROTOCOL_VERSION_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackRejectedIdentifierRejectedTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::IDENTIFIER_REJECTED_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackRejectedServerUnavailableTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::SERVER_UNAVAILABLE_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackRejectedBadUserdataTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::BAD_USERDATA_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackRejectedNotAuthorizedTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::NOT_AUTHORIZED_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, HandleConnackInvalidReturnCodeTest) {
				...
				p_network_connection_->SetNextReadBuf(TestHelper::GetSerializedConnAckMessage(false, ConnackTestReturnCode::INVALID_VALUE_ERROR));
			...
			TEST_F(ConnectDisconnectActionTester, KeepAliveSendPingreqTest) {
				...
				EXPECT_CALL(*p_network_mock_, WriteInternalProxy(::testing::_, ::testing::_)).WillOnce(::testing::DoAll(::testing::SetArgReferee<1>(p_pingreq_packet->Size()), ::testing::Return(ResponseCode::SUCCESS)));
			...
			TEST_F(ConnectDisconnectActionTester, KeepAlivePingrespNotReceivedTest) {
				...
				p_core_state_->RegisterAction(ActionType::DISCONNECT, mqtt::DisconnectActionAsync::Create, p_core_state_);
